...
public:
	...
	void AddDocument(int document_id, const string& document) {
		...
		document_count_++; // Подсчёт количества документов
...
private:
	...
	vector<Document> FindAllDocuments(const Query& query_words) const {
		...
		for (const string& word : query_words.plus_words) {
			...
			idf_for_word = log(document_count_ / static_cast<double>(word_to_document_freqs_.at(word).size())); // Расчёт IDF для слова word
			
			for (const auto& doc : word_to_document_freqs_.at(word)) {
				document_to_relevance[doc.first] += doc.second * idf_for_word;
			}
		}
